...
export function Navbar() {
  ...
  const { menuData, mainNav } = navigationConfig;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex justify-between items-center transition-all duration-300 ${scrolled ? 'h-16' : 'h-20'}`}>
          ...
          <div className="hidden md:flex items-center">
            {mainNav.map((link) => {
              const menu = menuData[link.key as keyof typeof menuData];
              const isDirectLink = !menu;
              ...
              return (
                <div 
                  ...
                  onMouseEnter={() => menu && setActiveMenu(link.key)}
                >
                  <div className="relative cursor-pointer py-2 text-[#11253e]">
                    {isDirectLink && link.href ? (
                      <Link href={link.href}>
                        {LabelContent}
                      </Link>
                    ) : (
                    ...
                    {activeMenu === link.key && menu && (
                      ...
                      {menu.type === 'mega' ? (
                        <div className={`grid grid-cols-3 ${link.key === 'SERVICES' ? 'gap-6' : 'gap-10'}`}>
                          {menu.columns.map((col: any, i: number) => (
              ...
              {mainNav.map((link) => {
                ...
                return (
                  <div key={link.key} className="border-b border-[#11253e]/5 pb-6">
                    ...
                    >
                      {isDirectLink && link.href ? (
                        <Link href={link.href} className="text-[16px] font-bold tracking-tight text-[#11253e] uppercase">
                          {link.label}
                        </Link>
                      ) : (
                  ...
                  {expandedMobileMenu === link.key && menu && (
                    <Motion.div 
